...
뉴스 키워드 빈도 vs 워크넷 채용공고 키워드 빈도 비교표/막대차트
"""
...
def plot_discourse_vs_reality(
    news_freq: pd.Series,
    worknet_freq: pd.Series,
    top_n: int = 20,
    save_path: str | None = None,
) -> None:
    """뉴스 담론과 워크넷 요구역량 상위 N개 나란히 비교."""
    fig, axes = plt.subplots(1, 2, figsize=(16, 8))

    news_top = news_freq.head(top_n)
    worknet_top = worknet_freq.head(top_n)

    axes[0].barh(news_top.index[::-1], news_top.values[::-1], color="#4c72b0")
    axes[0].set_title("뉴스 담론 상위 키워드 (빅카인즈)", fontsize=13)

    axes[1].barh(worknet_top.index[::-1], worknet_top.values[::-1], color="#dd8452")
    axes[1].set_title("실제 채용공고 요구역량 (워크넷)", fontsize=13)
